...
def auth_test():
    ...
    ifl.load_auths(email="email@gmail.com")
...
def post_test():
    ...
    example_post = ifl.Post("KDI1pp1g7")
    ...
    assert example_post.creator_name == 'I_love_dogs'
...
def comment_comaprison():
    ...
    response = ifl.post_from_url(
        'https://ifunny.co/video/tiktok-how-doesiit-feel-protect-at-all-costs-dGfZ0ZpN9')
    comment = ifl.api_call(f"https://api.ifunny.mobi/v4/content/{response.id}/comments", auth=ifl.BASIC_TOKEN)['data'][
        'comments']['items'][0]
    comment_two = ifl.Comment('AU9c9obi7', '5edbbfcfe6abce6b205106ec')
...
def comment_test():
    ...
    example_comment = ifl.Comment(post_id="NSRM2Edh7", comment_id='5ed3d6beec57403d3d034ce7')
...
def get_post_info_and_comments():
    ...
    # print(ifl.post_from_url('https://ifunny.co/video/tiktok-how-doesiit-feel-protect-at-all-costs-dGfZ0ZpN9').post_id) returned GXxybkXN7
    example_post = ifl.post_from_id('GXxyakXN7')
...
def basic_queue_loading():
    ...
    author_id = ifl.get_author_id("time69882time6988")
...
def get_subscriptions():
    ...
    #
    author_id = ifl.get_author_id("timetime")
    ...
    response = ifl.api_call(url=f"https://api.ifunny.mobi/v4/users/{author_id}/subscriptions", auth=ifl.BASIC_TOKEN,
                            params={"limit": 1})
    sub_author_id = "51cb699ba573a4904eb0a0be"
...
def deleted_comment():
    com = ifl.Comment('fN8odWnz7', '5f68130432cf2f798314db6e')
    pprint(com.cdata)
...
def denester():
    example_post = ifl.post_from_id('GXxyboXN7')
    example_post.load_comment_tree(limit=2, reply_limit=1)


def endpoint_test():
    post = ifl.post_from_id("GXxyboXN7")
    ...
    response = ifl.api_call(
        url=f"https://api.ifunny.mobi/v4/content/NSRM2Edh7/comments/5ed40dcef4638846d3662a2b",
        auth=ifl.BEARER_TOKEN,
        params={"limit": 3})
    pprint(response)
